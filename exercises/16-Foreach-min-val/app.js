var myArray = [3344,34334,454543,342534,4563456,3445,23455,234,262,2335,43323,4356,345,4545,452,345,434,36,345,4334,5454,345,4352,23,365,345,47,63,425,6578759,768,834,754,35,32,445,453456,56,7536867,3884526,4234,35353245,53244523,566785,7547,743,4324,523472634,26665,63432,54645,32,453625,7568,5669576,754,64356,542644,35,243,371,3251,351223,13231243,734,856,56,53,234342,56,545343];

//your code here
//1. Declare an auxiliar global variable called veryBigInteger 
//2. Set the value to the veryBigInteger variable
//3. Create a loop
//4. Inside the loop, check each element to see if it is smaller than veryBigInteger 
//(compare element to the variable)
//5. If it is smaller, than assign the variable to the element
//6. Outside the loop, after loop is finished, print the auxiliar value

var veryBigInteger = 